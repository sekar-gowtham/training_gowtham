	...
	while (1)
	{
		...
		case 6:
			if (1)
			{
				...
				xml_attribute<>*attribute;
				xml_attribute<>*attribute1;
				newnode = doc.allocate_node(node_element, "artist", "");
				cout << "\nEnter artist id ";
				cin >> artist_id;
				attribute = doc.allocate_attribute("id", artist_id);
				newnode->append_attribute(attribute);
				cout << "\nEnter artist name ";
				cin.ignore();
				gets_s(artist_name);
				child = doc.allocate_node(node_element, "artist_name", artist_name);

				child1 = doc.allocate_node(node_element, "songsOfArtist", "");
				cout << "\nEnter songs reference ";
				gets_s(song_refs);
				attribute1 = doc.allocate_attribute("idrefs", song_refs);
				child1->append_attribute(attribute1);
				newnode->append_node(child);
				newnode->append_node(child1);
				node1->append_node(newnode);
		...
		case 7:
			if (1)
			{
				...
				xml_attribute<>*attribute;
				xml_attribute<>*attribute1;
				newnode = doc.allocate_node(node_element, "album", "");
				cout << "\nEnter album id ";
				cin >> album_id;
				attribute = doc.allocate_attribute("id", album_id);
				newnode->append_attribute(attribute);
				cout << "\nEnter album name ";
				cin.ignore();
				gets_s(album_name);
				child = doc.allocate_node(node_element, "album_name", album_name);

				child1 = doc.allocate_node(node_element, "songsOfAlbum", "");
				cout << "\nEnter songs reference ";
				gets_s(song_refs);
				attribute1 = doc.allocate_attribute("idrefs", song_refs);
				child1->append_attribute(attribute1);
		...
		case 8:
			if (1)
			{
				...
				xml_attribute<>*attribute;
				xml_attribute<>*attribute1;
				newnode = doc.allocate_node(node_element, "playlist", "");
				cout << "\nEnter playlist id ";
				cin >> playlist_id;
				attribute = doc.allocate_attribute("id", playlist_id);
				newnode->append_attribute(attribute);
				cout << "\nEnter playlist name ";
				cin.ignore();
				gets_s(playlist_name);
				child = doc.allocate_node(node_element, "playlist_name", playlist_name);

				child1 = doc.allocate_node(node_element, "songsOfPlaylist", "");
				cout << "\nEnter songs reference ";
				gets_s(song_refs);
				attribute1 = doc.allocate_attribute("idrefs", song_refs);
				child1->append_attribute(attribute1);
