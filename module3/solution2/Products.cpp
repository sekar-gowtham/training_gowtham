...
int Products::searchPrice(Products *temp, float price)
{
	int f = 0;
	...
	if (temp != NULL)
	{
		...
		{
			f = searchPrice(temp->left, price);
		}
		else if (temp->product_price < price)
		{
			f = searchPrice(temp->right, price);										//if price is greater than the price of the node move right side of the tree
		}
		else if (temp->product_price == price)
		{
...
void Products::numberOfProducts(Products *root)
{
	int size = 0;
	float price;
	if (0 == flag)
